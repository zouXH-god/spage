...
type OIDCConfig struct {
	gorm.Model
	AdminGroups      []string `gorm:"column:admin_groups"`       // 平台管理员组，默认为：[]string{}，*为匹配所有组
	AllowedGroups    []string `gorm:"column:allowed_groups"`     // 允许登录的组，默认为：[]string{"*"}，*为匹配所有组
	...
	GroupsClaim      *string  `gorm:"column:groups_claim"`       // 组声明，默认为："groups"
